...
class ModelExplainer:
        ...
        try:
            if model_type == "tree":
                ...
                self.shap_explainer = shap.PermutationExplainer(
                    f, X_background, max_evals=max_evals
                )
            ...
            logger.warning(
                f"Primary SHAP explainer creation failed ({e}); falling back to permutation explainer."
            )
            try:
                f = (
                    model.predict_proba
    ...
    def explain_prediction_shap(
        ...
        try:
            ...
            if self.shap_explainer is None:
                ...
                X_background = (
                    X_instance.sample(background_size, random_state=42)
                    if len(X_instance) > background_size
                    else X_instance
                )
            ...
            top_features = [
                (feature_names[i], feature_importance[i]) for i in sorted_idx[:10]
            ]
    ...
    def explain_prediction_lime(
        self, model, X_instance: pd.DataFrame, num_features: int = 10
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            explanation = self.lime_explainer.explain_instance(
                X_instance.values[0], predict_proba, num_features=num_features
            )
    ...
    def global_feature_importance_shap(
        self,
        model,
        X_data: pd.DataFrame,
        model_type: str = "tree",
        ...
        try:
            ...
            explainer = self.create_shap_explainer(
                model, X_background, model_type, max_evals
            )
            ...
            try:
                ...
                logger.warning(
                    f"SHAP values failed with primary explainer ({e}); using permutation fallback."
                )
                f = (
                    model.predict_proba
                    if hasattr(model, "predict_proba")
                ...
                with self.mlflow.start_run(run_name="SHAP Global Explanation"):
                    ...
                    for i, (_, row) in enumerate(importance_df.head(20).iterrows()):
                        self.mlflow.log_metrics(
                            {f"feature_{i + 1}_importance": row["importance"]}
                        )
    ...
    def plot_shap_waterfall(
        ...
        model_type: str = "tree",
        max_evals: int = 1000,
        filename: str = "shap_waterfall_plot.png",
    ) -> bool:
        """
        ...
        try:
            explanation = self.explain_prediction_shap(
                model, X_instance, model_type, max_evals
            )
    ...
    def compare_models_shap(
        ...
        filename: str = "shap_model_comparison.png",
    ) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            for model_name, model in models.items():
                model_type = (
                    model_types.get(model_name, "tree") if model_types else "tree"
                )

                importance_df = self.global_feature_importance_shap(
                    model, X_data, model_type, filename=f"shap_{model_name}_summary.png"
                )

                if importance_df is not None:
                    importance_results[model_name] = importance_df.set_index("feature")[
                        "importance"
                    ]
                else:
                    # Fallback: use model-native feature importance so chart isn't empty
                    fallback_df = self._fallback_feature_importance(
                        model, X_data.columns.tolist(), model_name
                    )
                    if fallback_df is not None and not fallback_df.empty:
                        logger.warning(
                            f"Using fallback feature importance for {model_name}"
                        )
                        importance_results[model_name] = fallback_df.set_index(
                            "feature"
                        )["importance"]
    ...
    def _fallback_feature_importance(
        ...
        try:
            ...
            if len(importance) != len(feature_names):
                ...
                if len(importance) < len(feature_names):
                    importance = np.pad(
                        importance, (0, len(feature_names) - len(importance))
                    )
                else:
