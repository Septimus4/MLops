...
class DataPreprocessor:
    ...
    def handle_missing_values(
        self, df: pd.DataFrame, fill_value: float = -999
    ) -> pd.DataFrame:
        """
    ...
    def encode_categorical_features(
        self, df: pd.DataFrame, method: str = "label", max_categories: int = 30
    ) -> pd.DataFrame:
        """
        ...
        if method == "label":
            for col in categorical_cols:
                if df[col].nunique() <= max_categories:
                    ...
                    top_categories = value_counts.head(
                        max_categories - 1
                    ).index.tolist()
                    ...
                    # Apply mapping
                    df[col] = (
                        df[col].map(mapping).fillna(max_categories - 1).astype(int)
                    )
                    logger.info(
                        f"Encoded {col} with {len(mapping)} categories (including OTHER)"
                    )
        elif method == "onehot":
            # Select categorical columns with reasonable number of categories
            selected_cat_cols = [
                c for c in categorical_cols if df[c].nunique() <= max_categories
            ]
    ...
    def prepare_main_dataset(
        self, encoding_method: str = "label", sample_size: Optional[float] = None
    ) -> tuple:
        """
    ...
    def create_train_val_test_split(
        ...
        logger.info(
            f"Train set: {X_train.shape}, Val set: {X_val.shape}, Test set: {X_test.shape}"
        )

        return X_train, X_val, X_test, y_train, y_val, y_test

    def get_feature_importance_baseline(
        self, X: pd.DataFrame, y: pd.Series, n_estimators: int = 50
    ) -> pd.DataFrame:
        """
