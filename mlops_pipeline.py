...
class MLOpsPipeline:
    ...
    def __init__(
        self,
        data_dir: str = "home-credit-default-risk-DATA",
    ...
    def run_data_preparation(self, sample_size: float = 0.3):
        ...
        X, y, feature_names = self.data_preprocessor.prepare_main_dataset(
            sample_size=sample_size
        )
        X_train, X_val, X_test, y_train, y_val, y_test = (
            self.data_preprocessor.create_train_val_test_split(X, y)
        )
    ...
    def run_model_training(self, X_train, y_train, X_val, y_val):
        ...
        for model_name in [
            ...
            cv_result = self.model_trainer.cross_validate_model(
    ...
    def select_best_model(self, cv_results, trained_models):
        ...
        best_model_name = max(
            cv_results.keys(), key=lambda x: cv_results[x]["mean_auc"]
        )
    ...
    def run_complete_pipeline(self):
        ...
        try:
            X_train, X_val, X_test, y_train, y_val, y_test, feature_names = (
                self.run_data_preparation(sample_size=0.3)
            )
            ...
            if self.mlflow_manager:
                with self.mlflow_manager.start_run(run_name="Full Pipeline"):
                    ...
                    try:
                        ...
                        self.mlflow_manager.log_input_dataset(
                            train_df.head(5000), name="train", context="training"
                        )
                        self.mlflow_manager.log_input_dataset(
                            val_df.head(2000), name="validation", context="validation"
                        )
                        self.mlflow_manager.log_input_dataset(
                            test_df.head(2000), name="test", context="test"
                        )
                    ...
                    trained_models, cv_results = self.run_model_training(
                        X_train, y_train, X_val, y_val
                    )
            ...
            y_val_proba = self.model_trainer.get_model_predictions(
                best_model_name, X_val
            )
            thr_metrics = optimize_threshold_by_cost(
                y_val, y_val_proba, fn_cost=5.0, fp_cost=1.0
            )

            # Log thresholding decision
            try:
                with self.mlflow_manager.start_run(
                    run_name="Threshold Optimization", nested=True
                ):
            ...
            deployment_success = self.run_model_deployment(
                best_model, best_model_name, metadata
            )
            ...
            try:
                ...
                if self.mlflow_manager:
                    with self.mlflow_manager.start_run(
                        run_name="Pipeline Summary", nested=True
                    ):
                        self.mlflow_manager.log_artifact(
                            str(out_path), artifact_path="reports"
                        )
            except Exception as e:
            ...
            logger.info(
                f"Deployment Status: {'Success' if deployment_success else 'Failed'}"
            )
            logger.info("=" * 50)
            return {
                "best_model_name": best_model_name,
...
def test_optimized_pipeline():
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = (
        data_preprocessor.create_train_val_test_split(X, y, test_size=0.2, val_size=0.2)
    )
...
def main():
    ...
    exp_p = subparsers.add_parser(
        "explain", help="Run only explainability: prep + (subset) train + SHAP/LIME"
    )
    exp_p.add_argument(
        "--data-dir",
        default="home-credit-default-risk-DATA",
    ...
    exp_p.add_argument(
        ...
        type=float,
        default=0.1,
        help="Fraction of data to sample for speed (0-1]",
    )
    exp_p.add_argument(
        "--models",
        default=None,
    ...
    exp_p.add_argument(
        ...
        type=int,
        default=500,
        help="Rows to use for SHAP/LIME (smaller = faster)",
    )
    ...
    if cmd == "explain":
        ...
        X_train, X_val, X_test, y_train, y_val, y_test, feature_names = (
            pipeline.run_data_preparation(sample_size=args.sample_size)
        )
        ...
        if args.models:
            ...
            for name in requested:
                if name == "logistic_regression":
                    models[name] = mt.train_logistic_regression(
                        X_train, y_train, X_val, y_val, start_mlflow_run=False
                    )
                elif name == "random_forest":
                    models[name] = mt.train_random_forest(
                        X_train, y_train, X_val, y_val, start_mlflow_run=False
                    )
                elif name == "lightgbm":
                    models[name] = mt.train_lightgbm(
                        X_train, y_train, X_val, y_val, start_mlflow_run=False
                    )
                elif name == "xgboost":
                    models[name] = mt.train_xgboost(
                        X_train, y_train, X_val, y_val, start_mlflow_run=False
                    )
                elif name == "mlp":
                    models[name] = mt.train_mlp(
                        X_train, y_train, X_val, y_val, start_mlflow_run=False
                    )
                else:
        ...
        comparison_df = pipeline.run_model_explainability(
            models, X_for_explain, feature_names
        )
        print(
            "Explainability run complete. Check MLflow for 'SHAP Model Comparison' artifacts."
        )
        if comparison_df is not None:
            print("Top 10 combined features:\n", comparison_df.head(10))
        return {
            "comparison": comparison_df.head(10).to_dict()
            if comparison_df is not None
            else None
        }
    ...
    data_dir = (
        args.data_dir if hasattr(args, "data_dir") else "home-credit-default-risk-DATA"
    )
    if not os.path.exists(data_dir):
        logger.error(f"Data directory {data_dir} not found!")
        sys.exit(1)
