...
class ModelTrainer:
        ...
        if X_val is not None and y_val is not None:
            ...
            if self.mlflow and start_mlflow_run:
                with self.mlflow.start_run(run_name="Logistic Regression"):
                    self.mlflow.log_input_dataset(
                        pd.concat([X_train, y_train], axis=1).head(100),
                        name="train_sample",
                        context="training",
                    )
                    self.mlflow.log_input_dataset(
                        pd.concat([X_val, y_val], axis=1).head(100),
                        name="val_sample",
                        context="validation",
                    )
        ...
        if X_val is not None and y_val is not None:
            ...
            if self.mlflow and start_mlflow_run:
                with self.mlflow.start_run(run_name="Random Forest"):
                    ...
                    self.mlflow.log_metrics({"val_auc": auc})
                    self.mlflow.log_model(
                        model, "random_forest", "sklearn", input_example=X_val.head(5)
                    )
        ...
        if X_val is not None and y_val is not None:
            ...
            if self.mlflow and start_mlflow_run:
                with self.mlflow.start_run(run_name="LightGBM"):
                    ...
                    self.mlflow.log_metrics({"val_auc": auc})
                    self.mlflow.log_model(
                        model, "lightgbm", "lightgbm", input_example=X_val.head(5)
                    )
        ...
        if dval is not None and X_val is not None and y_val is not None:
            ...
            if self.mlflow and start_mlflow_run:
                with self.mlflow.start_run(run_name="XGBoost"):
                    ...
                    self.mlflow.log_metrics({"val_auc": auc})
                    self.mlflow.log_model(
                        model, "xgboost", "xgboost", input_example=X_val.head(5)
                    )
        ...
        if X_val is not None and y_val is not None:
            ...
            if self.mlflow and start_mlflow_run:
                with self.mlflow.start_run(run_name="MLP Classifier"):
                    ...
                    self.mlflow.log_metrics({"val_auc": auc})
                    self.mlflow.log_model(
                        model, "mlp", "sklearn", input_example=X_val.head(5)
                    )
    ...
    def cross_validate_model(
        ...
        """
        logger.info(
            f"Performing {self.cv_folds}-fold cross-validation for {model_name}"
        )
        ...
        if model_name == "logistic_regression":
            ...
            def lgb_cv(Xi, yi):
                ...
                for tr, va in skf.split(Xi, yi):
                    ...
                    lgb_va = lgb.Dataset(
                        Xi.iloc[va], label=yi.iloc[va], reference=lgb_tr
                    )
                    booster = lgb.train(
                        params,
                        lgb_tr,
                    ...
                    pred = np.asarray(
                        booster.predict(
                            Xi.iloc[va], num_iteration=booster.best_iteration
                        )
                    ).ravel()
            ...
            logger.info(
                f"{model_name} CV AUC: {results['mean_auc']:.4f} (+/- {results['std_auc']:.4f})"
            )
            return results
            ...
            def xgb_cv(Xi, yi):
                ...
                for tr, va in skf.split(Xi, yi):
                    ...
                    pred = np.asarray(
                        booster.predict(
                            dva, iteration_range=(0, booster.best_iteration)
                        )
                    ).ravel()
                    scores.append(roc_auc_score(yi.iloc[va], pred))
                return np.array(scores)
        ...
        if self.mlflow:
            with self.mlflow.start_run(run_name="All Models Training"):
                ...
                if X_train is not None and y_train is not None:
                    self.mlflow.log_input_dataset(
                        pd.concat([X_train, y_train], axis=1).head(200),
                        name="train_sample",
                        context="training",
                    )
                if X_val is not None and y_val is not None:
                    self.mlflow.log_input_dataset(
                        pd.concat([X_val, y_val], axis=1).head(200),
                        name="val_sample",
                        context="validation",
                    )
                ...
                input_example = (
                    ...
                    else (X_train.head(5) if X_train is not None else None)
                )
